				...
				const char *env_prefix, const char *env_uri)
{
	...
	soup_message_set_uri (SOUP_MESSAGE (msg), (SoupURI *) uri);
...
soup_soap_message_parse_response (SoupSoapMessage *msg)
{
	...
	xmlstr = g_malloc0 (SOUP_MESSAGE (msg)->response_body->length + 1);
	strncpy (xmlstr, SOUP_MESSAGE (msg)->response_body->data, SOUP_MESSAGE (msg)->response_body->length);
