...
/**
 * soup_soap_parameter_get_boolean_value:
 * @param: the parameter
 *
 * Returns the parameter's (boolean) value.
 *
 * Return value: the parameter value as a gboolean
 */
gboolean
soup_soap_parameter_get_boolean_value (SoupSoapParameter *param)
{
	gboolean b;
	xmlChar *s;
	g_return_val_if_fail (param != NULL, -1);
	...
	s = xmlNodeGetContent (param);
	if (s) {
		b = (strcmp ((char *)s, "true") == 0)? TRUE : FALSE;
		xmlFree (s);
