...
static void
update_questions_tree_view (GPInstructLessonTestTextEditor *editor)
{
	GPInstructLessonTestTextEditorPrivate *priv = editor->priv;
	...
	GList *questions = gpinstruct_lesson_test_text_get_questions (priv->test);
	GList *curr_questions = questions;

	while (curr_questions)
	{
		GPInstructLessonTestTextQuestion *question = GPINSTRUCT_LESSON_TEST_TEXT_QUESTION (curr_questions->data);

		text = gpinstruct_lesson_test_text_question_get_text (question);
...
questions_tree_view_row_activated (GtkTreeView       *tree_view,
                                   ...
                                   gpointer           user_data)
{
	GPInstructLessonTestTextEditor *editor = GPINSTRUCT_LESSON_TEST_TEXT_EDITOR (user_data);
	GPInstructLessonTestTextEditorPrivate *priv = editor->priv;
	...
	if (gtk_tree_model_get_iter (GTK_TREE_MODEL (priv->questions_store), &iter, path))
	{
		gtk_tree_model_get (GTK_TREE_MODEL (priv->questions_store), &iter,
		                    DATA_COLUMN, &object,
		                    -1);
		if (GPINSTRUCT_IS_LESSON_TEST_TEXT_QUESTION (object))
		{
			...
			gtk_text_buffer_set_text (gtk_text_view_get_buffer (GTK_TEXT_VIEW (explanation_view)),
			                          gpinstruct_lesson_test_text_question_get_explanation (question),
			                          -1);
			...
			gtk_text_buffer_set_text (gtk_text_view_get_buffer (GTK_TEXT_VIEW (answer_view)),
			                          ...
			                          -1);
			gtk_container_add (GTK_CONTAINER (scrolled_window), answer_view);
			gtk_box_pack_start (GTK_BOX (content_area),
			...
			if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)
			{
				...
				gpinstruct_lesson_test_text_question_set_text (question, text);
				g_free (text);

				gtk_text_buffer_get_bounds (gtk_text_view_get_buffer (GTK_TEXT_VIEW (explanation_view)),
				                            &start, &end);
				text = gtk_text_iter_get_text (&start, &end);
				gpinstruct_lesson_test_text_question_set_explanation (question, text);
				g_free (text);

				gtk_text_buffer_get_bounds (gtk_text_view_get_buffer (GTK_TEXT_VIEW (answer_view)),
				                            &start, &end);
				text = gtk_text_iter_get_text (&start, &end);
				gpinstruct_lesson_test_text_question_set_answer (question, text);
				g_free (text);

				update_questions_tree_view (GPINSTRUCT_LESSON_TEST_TEXT_EDITOR (user_data));
                              ...
                              gpointer   user_data)
{
	...
	question = gpinstruct_lesson_test_text_question_new ();
	gpinstruct_lesson_test_text_add_question (priv->test, question);
                                 ...
                                 gpointer   user_data)
{
	...
	if (gtk_tree_selection_get_selected (selection, NULL, &iter))
	{
		...
		gpinstruct_lesson_test_text_remove_question (priv->test, gtk_tree_path_get_indices (path)[0]);
...
G_DEFINE_TYPE (GPInstructLessonTestTextEditor, gpinstruct_lesson_test_text_editor, GPINSTRUCT_TYPE_OBJECT_EDITOR);
...
gpinstruct_lesson_test_text_editor_init (GPInstructLessonTestTextEditor *object)
{
	...
	GPInstructLessonTestTextEditorPrivate *priv = object->priv;
...
static void
gpinstruct_lesson_test_text_editor_finalize (GObject *object)
{
	GPInstructLessonTestTextEditor *editor = GPINSTRUCT_LESSON_TEST_TEXT_EDITOR (object);
	GPInstructLessonTestTextEditorPrivate *priv = editor->priv;

	if (priv->questions_store)
		g_object_unref (priv->questions_store);

	G_OBJECT_CLASS (gpinstruct_lesson_test_text_editor_parent_class)->finalize (object);
}

static void
gpinstruct_lesson_test_text_editor_class_init (GPInstructLessonTestTextEditorClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	/*GPInstructObjectEditorClass *parent_class = GPINSTRUCT_OBJECT_EDITOR_CLASS (klass);*/

	g_type_class_add_private (klass, sizeof (GPInstructLessonTestTextEditorPrivate));

	object_class->finalize = gpinstruct_lesson_test_text_editor_finalize;
}
...
GPInstructLessonTestTextEditor *
gpinstruct_lesson_test_text_editor_new (GPInstructEditorWindow *window,
                                        GPInstructLessonTestText *test)
{
	GPInstructLessonTestTextEditor *editor = g_object_new (GPINSTRUCT_TYPE_LESSON_TEST_TEXT_EDITOR, NULL);
	GPInstructLessonTestTextEditorPrivate *priv = editor->priv;
