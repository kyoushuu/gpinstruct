	...
	if (gtk_tree_model_get_iter (GTK_TREE_MODEL (priv->questions_store), &iter, path))
	{
		...
		if (GPINSTRUCT_IS_LESSON_TEST_SCRAMBLED_QUESTION (object))
		{
			...
			if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)
			{
				...
				gpinstruct_lesson_test_scrambled_question_set_explanation (question, text);
				g_free (text);

				gtk_text_buffer_get_bounds (gtk_text_view_get_buffer (GTK_TEXT_VIEW (answer_view)),
				                            &start, &end);
				text = gtk_text_iter_get_text (&start, &end);
				gpinstruct_lesson_test_scrambled_question_set_answer (question, text);
				g_free (text);
...
G_DEFINE_TYPE (GPInstructLessonTestScrambledEditor, gpinstruct_lesson_test_scrambled_editor, GPINSTRUCT_TYPE_OBJECT_EDITOR);
...
gpinstruct_lesson_test_scrambled_editor_class_init (GPInstructLessonTestScrambledEditorClass *klass)
{
	...
	object_class->finalize = gpinstruct_lesson_test_scrambled_editor_finalize;
}
